...
import { subscribeAccommodation, addAccommodation, updateAccommodation, deleteAccommodation } from '../db.js';
      ...
      <div class="card" style="margin:0 16px 12px;cursor:pointer" onclick="window.__editAccomItem('${item.id}')">
        <div class="card-body" style="padding:14px">
          ...
          <div class="row gap-16">
            <div>
              <div class="eyebrow" style="margin-bottom:2px">Check-in</div>
              <div class="text-sm">${item.checkIn || '—'}</div>
            </div>
            <div>
              <div class="eyebrow" style="margin-bottom:2px">Check-out</div>
              <div class="text-sm">${item.checkOut || '—'}</div>
            </div>
            ${nights !== null ? `
            <div style="margin-left:auto">
              <div class="eyebrow" style="margin-bottom:2px">Nights</div>
...
function openItemModal(item) {
  ...
  openModal({
    title: isEdit ? 'Edit Stay' : t('accom.add'),
    body: `
      <form id="accom-form">
        ...
        <div class="form-row">
          <div class="form-group" style="flex:2">
            <label class="form-label">${t('accom.cost')} (total)</label>
  ...
  window.__saveAccomItem = async (id) => {
    ...
    try {
      if (id) {
        await updateAccommodation(_ctx.userId, _ctx.tripId, id, data);
        showToast('Stay updated');
      } else {
        await addAccommodation(_ctx.userId, _ctx.tripId, data);
  ...
  window.__deleteAccomItem = async (id) => {
    ...
    if (!confirmed) return;
    try {
      await deleteAccommodation(_ctx.userId, _ctx.tripId, id);
